...
const PRINCIPLES: Array<[string, string]> = [
  ...
  [
    "Inverted block hover",
    "background: var(--phosphor); color: var(--bg) — no transitions.",
  ],
...
const SectionTitle: React.FC<{ children: React.ReactNode }> = ({
  ...
  <h2
    style={{
      ...
      background:
        "linear-gradient(90deg, rgba(44,255,122,0.08), transparent 60%)",
...
export const Introduction = () => (
  ...
  >
    ...
    <p style={{ color: "var(--phosphor-dim)", maxWidth: 720, marginTop: 0 }}>
      Single-channel green phosphor. Retro / military / 80s terminal / Pip-Boy
      3000 / STALKER GAMMA aesthetic. Mono fonts, hard edges, glow instead of
      shadow.
    </p>
...
import { Button, PdaWindow, PostListing } from "@sektant1/phosphor-ui";`}</Code>
    ...
    <FeatureList
      items={PRINCIPLES.map(([title, body]) => ({
        title,
        body,
      }))}
    />
    ...
    >
      <div>
        ▸ <span style={{ color: "var(--phosphor-bright)" }}>Zone / Tokens</span>{" "}
        — colors, type, spacing, scrollbar, z-index.
      </div>

      <div>
        ▸{" "}
        <span style={{ color: "var(--phosphor-bright)" }}>Components / *</span>{" "}
        — every component, isolated.
      </div>

      <div>
        ▸{" "}
        <span style={{ color: "var(--phosphor-bright)" }}>
          Pages / Demo Site
        </span>{" "}
